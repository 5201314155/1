...
## 2026-01-06 Compose Foundation 依赖缺失导致编译失败
- 复现：Termux 环境（Gradle 8.7 + JDK 21 + ARM64 aapt2）在 `:app:compileDebugKotlin` 报 `align`/`weight` 未解析，原因是基础布局扩展未随 BOM 引入。
- 处理：在 `app/build.gradle.kts` 增加 `androidx.compose.foundation:foundation` 与 `androidx.compose.foundation:foundation-layout` 依赖，确保 `Modifier.weight`/`Modifier.align` 等布局扩展从官方 BOM 正常解析；请在手机端重跑 `bash run.sh 3` 或 `bash run.sh 5` 验证 Kotlin 编译通过，资源阶段继续使用本机 aapt2 覆盖。

## 2026-01-07 一键尺寸重设的编译记录
- 状态：容器内缺少 Android SDK/Termux 环境，未运行实际构建；新增的尺寸重设按钮仅修改 Compose 逻辑，不引入新的依赖。
- 建议：在 Termux 环境执行 `bash run.sh 3`（完整构建）或 `bash run.sh 5`（增量构建），验证 Kotlin 阶段无新增报错，资源阶段继续依赖本机 ARM64 aapt2 覆盖与隔离 `.gradle-mobile` 缓存策略。

## 2026-01-08 Compose 版本匹配与对齐渲染调整
- 问题：Termux 侧 Kotlin 编译仍偶发 `align/weight` 解析失败，怀疑 Compose BOM 与编译器扩展版本存在微小错配。
- 处理：`app/build.gradle.kts` 将 Compose BOM 固定为 2024.04.01，并将 `kotlinCompilerExtensionVersion` 升级到 1.6.8，以匹配当前 Kotlin 1.9.24/AGP 8.5.0 组合；画布对齐改用嵌套 Box 的 `contentAlignment`，避免依赖额外的 `Modifier.align` 扩展。
- 待验证：在手机端运行 `bash run.sh 3` 或 `bash run.sh 5`，确认 `:app:compileDebugKotlin` 不再提示 align/weight 相关错误，资源阶段继续沿用本机 ARM64 aapt2 覆盖策略。

## 2026-01-09 Compose BOM 校准与布局符号兜底
- 变更：将 Compose BOM 提升至 2024.06.00，以与 `kotlinCompilerExtensionVersion=1.6.8` 更紧密匹配，避免 `Modifier.weight`/`align` 被解析为内部 API。
- 处理：`MainActivity.kt` 明确补足布局相关 import，减少符号缺失导致的 Kotlin 编译中断。
- 状态：容器内仍无 Android SDK/Termux，未实际运行 `bash run.sh`；请在手机端执行菜单 3/5 复测 `:app:compileDebugKotlin` 已无 align/weight 报错，资源阶段继续使用本机 ARM64 aapt2 与隔离缓存。

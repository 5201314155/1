...
- 处理：在 `app/build.gradle.kts` 增加 `androidx.compose.foundation:foundation` 依赖，确保 `Modifier.weight`/`Modifier.align` 等布局扩展从官方 BOM 正常解析；请在手机端重跑 `bash run.sh 3` 或 `bash run.sh 5` 验证 Kotlin 编译通过，资源阶段继续使用本机 aapt2 覆盖。

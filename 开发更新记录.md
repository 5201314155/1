...
- `:app:compileDebugKotlin` 报 `align/weight` 未解析，原因是缺少 Compose Foundation 及布局子模块依赖，导致布局相关扩展函数无法从 BOM 中解析。
- 补充 `androidx.compose.foundation:foundation` 与 `androidx.compose.foundation:foundation-layout` 依赖，确保 `Modifier.weight` 与 `Modifier.align` 等布局扩展在手机端编译正常可用。

## 2026-01-07 一键尺寸贴合与分区安全区
- 属性面板新增“撑满可用宽度”“贴合当前区域”按钮，调用新的尺寸重设函数，按假手机可用宽度与分区高度自动调整组件尺寸，避免手动试错。
- 尺寸提示与校验卡片同步展示设备/分区限制，保证在移动端直接点按即可修正到安全尺寸，继续保持“画布=手机”的布局约束。

## 2026-01-08 Compose 依赖校准与对齐渲染修复
- 将 Compose BOM 调整为 2024.04.01，编译器扩展升级到 1.6.8，确保与 Termux 侧 Kotlin/AGP 组合稳定兼容，避免布局扩展函数因版本错配再度报错。
- 画布预览的对齐渲染改为基于 `contentAlignment` 的嵌套 Box，不再依赖 `Modifier.align` 扩展，减少在手机端编译时的符号解析风险，同时维持组件在假手机区域的对齐与留白展示。

## 2026-01-09 Compose 版本校准与布局 import 兜底
- Compose BOM 升级至 2024.06.00，保持与 `kotlinCompilerExtension=1.6.8` 的版本一致性，确保 Termux 环境下 `Modifier.weight`/`align` 等布局扩展由兼容版本的基础库提供，避免“internal in foundation.layout”报错。
- `MainActivity.kt` 统一补充布局相关 import，并贴合新版依赖结构，减少符号遗漏导致的编译失败风险。
